{******************************************************************************}
{                       CnPack For Delphi/C++Builder                           }
{                     中国人自己的开放源码第三方开发包                         }
{                   (C)Copyright 2001-2011 CnPack 开发组                       }
{                   ------------------------------------                       }
{                                                                              }
{            本开发包是开源的自由软件，您可以遵照 CnPack 的发布协议来修        }
{        改和重新发布这一程序。                                                }
{                                                                              }
{            发布这一开发包的目的是希望它有用，但没有任何担保。甚至没有        }
{        适合特定目的而隐含的担保。更详细的情况请参阅 CnPack 发布协议。        }
{                                                                              }
{            您应该已经和开发包一起收到一份 CnPack 发布协议的副本。如果        }
{        还没有，可访问我们的网站：                                            }
{                                                                              }
{            网站地址：http://www.cnpack.org                                   }
{            电子邮件：master@cnpack.org                                       }
{                                                                              }
{******************************************************************************}

unit CnPackRegister;
{* |<PRE>
================================================================================
* 软件名称：开发包基础库
* 单元名称：开发包公共注册单元
* 单元作者：CnPack开发组
* 备    注：
* 开发平台：PWin98SE + Delphi 5.0
* 兼容测试：PWin9X/2000/XP + Delphi 5/6
* 本 地 化：该单元中的字符串均符合本地化处理方式
* 单元标识：$Id: CnPackRegister.pas 761 2011-02-07 14:08:58Z liuxiao@cnpack.org $
* 修改记录：2002.04.18 V1.1
*                新增TCnCopyright属性编辑器申明
*           2002.04.08 V1.0
*                创建单元
================================================================================
|</PRE>}

interface

{$I CnPack.inc}

// 注释掉该行取消对系统默认字体的修改
// 默认方式下使用宋体、Size9 字体取代系统默认的字体
// 字体设置影响新建的窗体、IDE中部分窗口等
// {$DEFINE CN_MODIFY_DEFAULT_FONT}

uses
  Windows, Messages, SysUtils, Classes, Graphics,
  {$IFDEF COMPILER6_UP}
  DesignIntf, DesignEditors,
  {$ELSE}
  DsgnIntf,
  {$ENDIF}
  CnClasses, CnPropEditors;

procedure Register;
{* 控件、组件编辑器、属性编辑器注册过程}

implementation

procedure Register;
begin
  {$IFDEF CN_MODIFY_DEFAULT_FONT}
  with DefFontData do    // 修改系统默认字体
  begin
    Handle := 0;
    Height := -12;
    Pitch := fpDefault;
    Style := [];
    Charset := GB2312_CHARSET;
    Name := '宋体';
  end;
  {$ENDIF}

  RegisterPropertyEditor(TypeInfo(TCnCopyright), nil, 'About', TCnCopyrightProperty);
end;

end.
