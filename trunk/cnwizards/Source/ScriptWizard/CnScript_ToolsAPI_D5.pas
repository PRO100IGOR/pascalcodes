...
* 单元标识：$Id: CnScript_ToolsAPI_D5.pas 763 2011-02-07 14:18:23Z liuxiao@cnpack.org $
...
begin
  ...
  begin
    RegisterMethod('Function AddKeyBinding( const Keys : array of TShortCut; KeyProc : TKeyBindingProc; Context : Pointer; Flags : TKeyBindingFlags; const Keyboard : string; const HotKey : string) : Boolean', cdRegister);
...
procedure SIRegister_IOTAServices(CL: TPSPascalCompiler);
begin
  //with RegInterfaceS(CL,'IUnknown', 'IOTAServices') do
  with CL.AddInterface(CL.FindInterface('IUnknown'), IOTAServices, 'IOTAServices') do
  begin
...
begin
  ...
  begin
    ...
    RegisterMethod('Function GetGlyph : HICON', cdRegister);
  end;
end;
...
procedure SIRegister_IOTAModule(CL: TPSPascalCompiler);
begin
  //with RegInterfaceS(CL,'IOTAModule40', 'IOTAModule') do
  with CL.AddInterface(CL.FindInterface('IOTAModule40'), IOTAModule, 'IOTAModule') do
  begin
...
procedure SIRegister_IOTAModuleInfo(CL: TPSPascalCompiler);
begin
  //with RegInterfaceS(CL,'IUnknown', 'IOTAModuleInfo') do
  with CL.AddInterface(CL.FindInterface('IUnknown'), IOTAModuleInfo, 'IOTAModuleInfo') do
  begin
...
begin
  ...
  begin
    RegisterMethod('Function GetFormDesigner : IFormDesigner', cdRegister);
...
begin
  ...
  begin
    ...
    RegisterMethod('Function GetIPersistent : IPersistent', cdRegister);
    RegisterMethod('Function GetIComponent : IComponent', cdRegister);
  end;
end;
...
begin
  ...
  CL.AddTypeS('TOTAFileNotification', '( ofnFileOpening, ofnFileOpened, ofnFile'
    ...
    + 'ad, ofnProjectDesktopSave, ofnPackageInstalled, ofnPackageUninstalled )');
